...
class DBManager(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    fun addColonia(idColn: String, refRouts: Array<String>, colnColin: Array<String>) {
        val db = this.writableDatabase

        // Check if the colonia already exists
        val cursor: Cursor = db.rawQuery("SELECT * FROM $TABLE_NAME_COLONIAS WHERE $COLUMN_ID_COLN=?", arrayOf(idColn))
        if (cursor.count == 0) {
            // If the colonia doesn't exist, insert it into the table
            val values = ContentValues()
            values.put(COLUMN_ID_COLN, idColn)

            // Convert refRouts and colnColin arrays to strings
            val refRoutsStr = refRouts.joinToString("|")
            val colnColinStr = colnColin.joinToString("|")
            values.put(COLUMN_REF_ROUTS, refRoutsStr)
            values.put(COLUMN_COLN_COLIN, colnColinStr)

            db.insert(TABLE_NAME_COLONIAS, null, values)
        } else {
            // If the colonia already exists, update its refRouts and colnColin
            val values = ContentValues()
            ...
            db.update(TABLE_NAME_COLONIAS, values, "$COLUMN_ID_COLN=?", arrayOf(idColn))
        }
